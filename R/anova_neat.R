...
#'@param data_per_subject Data frame or name of data frame as string. Should contain all values (measurements/observations) in a single row per each subject.
#'@param values String; column name or names. Multiple column names are also to be given as a single string, separated by commas (e.g., \code{values = 'var1, var2, var3'}). (Spaces are ignored.) Each such column should contain a single dependent variable. This means, to test repeated (within-subject) measurements, each specified column should contain one measurement.
#'@param between_vars String; column name or names. Multiple column names are also to be given as a single string, separated by commas (e.g., \code{values = 'grouping1, grouping2'}). (Spaces are ignored.) Each such column should contain a single between-subject independent variable (representing a between-subject factor).
#'@param within_ids String (or number); optionally specifies one-sided tests (t and
...
#'@param ci Numeric; confidence level for returned CIs. (Otherwise 90% by default; Lakens, 2014; Steiger, 2004.)
...
#'@param welch If FALSE, calculates via \code{\link[ez:ezANOVA]{ez::ezANOVA}} in case of a single factor (one-way) between-subject design (i.e., same as in case of every other design). Otherwise (default), calculates Welch's ANOVA via \code{\link[stats:oneway.test]{stats::oneway.test}} in such cases (one-way between-subject).
#'
#'@param e_correction String: \code{'gg'}, \code{'hf'}, \code{'none'}, or empty. If set to \code{'gg'}, Greenhouse-Geisser correction is applied in case of repeated measures (regardless of violation of sphericity). If set to \code{'hf'}, Huynh-Feldt correction is applied. If set to \code{'none'}, no correction is applied. Otherwise (e.g. if left empty, \code{''}, as per default), Greenhouse-Geisser correction is applied when Mauchly's sphericity test is significant and the Greenhouse-Geisser epsilon is not larger than \code{.75}, while Huynh-Feldt correction is applied when Mauchly's sphericity test is significant and the Greenhouse-Geisser epsilon is larger than \code{.75} (see Girden, 1992).
#'
#'@details
...
#'\code{var1}.)
#'
#'@return Prints ANOVA statistics (including, for each model, F-test with partial eta squared and its CI, and BF, as specified via the corresponding parameters) in APA style. Furthermore, when
...
#'  is \code{NULL} when \code{auc_added} is \code{FALSE}. There are also two additional elements of the list. One is
...
#'
#'(The ezANOVA object values will likely differ from the printed information when using Welch's one-way ANOVA, since that is not calculated via \code{\link[ez:ezANOVA]{ez::ezANOVA}}, but via \code{\link[stats:oneway.test]{stats::oneway.test}}; see Note.)
#'
#' @note
#'
#'
#'Confidence intervals are calculated, using the F value, via
#'\code{\link[MBESS:conf.limits.ncf]{MBESS::conf.limits.ncf}}, coverting noncentrality parameters to partial eta squared as \code{x/(x+ df_nom+df_denom+1)} (Smithson, 2003).
#'
#'The Bayes factor is calculated via \code{\link[BayesFactor]{correlationBF}}.
#'
#'
#'If Levene's test is significant and the data is unbalanced (unequal group sample sizes), you should either consider the results carefully or choose a different test.
#'
#'@references
#'
